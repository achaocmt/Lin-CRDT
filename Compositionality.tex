...
A history $h$ satisfies causal-time-stamp, if for each operations $o_1,o_2,o_3,o_4$, such that $(o_1,o_2) \in \mathit{vis}$, $o_2$ and $o_3$ are of same object, $\mathit{ts}(o_2) < \mathit{ts}(o_3)$, $(o_3,o_4) \in \mathit{vis}$, and $o_1$ and $o_4$ are of same object, then, we have $\mathit{ts}(o_1) < \mathit{ts}(o_4)$. 
