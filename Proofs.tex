...
the arrival of a new operation, the execution of a
...
operations for the object in the system, it needs only be updated when
a \emph{new} operation arrives, therefore only the execution of an
\lstinline|atSource| portion of an operation engenders the extension
of $\alinord$\footnote{For readers familiars with proving classical
...
\begin{itemize}
%\setlength{\itemsep}{0.5pt}
\item[-] $\mathsf{Refinement}$: each downstream produced by an update
