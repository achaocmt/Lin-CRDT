...
The annotations included in the \lstinline|atSource| procedures describe the construction of the linearization. We describe updates of the linearization using a two-vocabulary formula, the primed, resp., unprimed, variables denoting values after, resp., before, the execution of the procedure. Therefore, the linearization is extended by appending operations, eventually rewritten using a ??? $\gamma$, when they execute the \lstinline|atSource| procedure.
...
We also have to argue that for each query $\alabel_{\mathsf{qr}}\in\{\alabellongind[readIds]{a}{R}{},\alabellongind[read]{}{A}{}\}$, the sequence $\alinord'\cdot \alabel_{\mathsf{qr}}$ where $\alinord'$ is the projection of $\alinord$ on the set of updates
...
In the case of CRDT implementations, such as RGA, that use timestamps for conflict resolution, the \crdtlinearization{} of their histories are consistent also 



 the timestamp plays an important role in conflict resolution.
