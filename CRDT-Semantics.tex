...
In this section, we propose the semantics of a state-based CRDT or a operation-based CRDT. Then we shows how to extract histories from execution, and the correctness of CRDT implementations. 
...
To deal with operation-based CRDT, we additionally record a happen-before relation and a message delivery relation into each configuration. Given a operation-based CRDT object $\mathit{obj}$ with $\Sigma$ be the set of local states, its semantics is defined as $\llbracket \mathit{obj} \rrbracket_{\mathit{op}} = (\mathit{Config},\mathit{config}_0,\Sigma',\rightarrow)$ as in \figurename~\ref{fig:the semantics of a operation-based CRDT object}.
...
\item[] $\begin{array}{l c}
   ...
   \begin{array}{c}
     ...
     S_1 = \{ (\mathit{mid}',\mathit{mid}) \vert (\mathit{mid'},r) \in \mathit{MsgDel} \}, S_2 = (\mathit{max}(\mathit{MsgHB},r), \mathit{mid})
   \end{array}}
...
Similarly, given an execution $l$ of $\llbracket \mathit{obj} \rrbracket_{\mathit{op}}$ of operation-based CRDT $\mathit{obj}$, we can obtain a corresponding history $\mathit{history}(l) = (\mathit{Op},\mathit{ro},\mathit{vis})$. Here $\mathit{Op}$ and $\mathit{ro}$ is same as the case of state-based CRDT, $\mathit{vis} = \mathit{del} \cup (\mathit{del} \cdot \mathit{ro})$, and $\mathit{del}$ is obtained as follows: $(o_1,o_2) \in \mathit{del}$, if $o_1$ and $o_2$ are of different replicas, $o_1$ is a update operation and sends a message with identifier $\mathit{mid}$, there exists a $\mathit{receive}(\mathit{mid},r)$ action, $\mathit{receive}(\mathit{mid},r)$ and $o_2$ happen on a same replica and $\mathit{receive}(\mathit{mid},r)$ happens earlier. Since operation-based CRDT sends the operation as message, the visibility relation just contains delivery relation and (possibly) replica order. Let $\mathit{history}(\llbracket \mathit{obj} \rrbracket_{\mathit{op}})$ be the set of histories of all executions of $\llbracket \mathit{obj} \rrbracket_{\mathit{op}}$. Then, a object is distributed linearizable, if each of its history is, as stated by the following:
