...
\subsection{Last-Writer-Win Register (LWW-Register) Implementation}
\label{subsec:last-writer-win regiseter (LWW-register) implementation}
...
\subsection{Two-phase Set (2P-Set) Implementation}
\label{subsec:two-phase set (2P-set) implementation}
...
\subsection{Last-Writer-Win-Element-Set (LWW-Element-Set) Implementation}
\label{subsec:last-writer-win-element-set (LWW-element-set) implementation}
...
Given a T-tree, the order $<_t$ is defined as follows: $tid_1 <_t tid_2$, if one of the following cases holds
...
We define the following function for a T-tree $tree$:
...
The payload of each replica is a T-tree $tree$.

To do $\alabelshort[{\tt addBetween}]{a,b,c}$, we first ensure $tree$ contains T-characters of $a$ and $c$, $tid_a$ and $tid_c$ are adjacent in $tree$, and no T-character in $tree$ has value $b$. Then, we calls method $newID(tid_a,tid_c,id)$ to generate a new T-identifier that is with disambiguator $id$ and its T-identifier is between $tid_a$ and $tid_c$ in $<_t$, and use this T-identifier to generate a new T-character $w_b$, and put $w_b$ into $tree$. Here $tid_a$ and $tid_c$ are the T-identifiers of T-characters of $a$ and $c$, respectively.

$newID(tid_1,tid_2,id)$ require that $tid_1$ and $tid_2$ are adjacent according to $<_t$ in $tree$, and it will generate a T-identifier $tid'$, such that $tid_1 <_t tid' <_t tid_2$.
...
%\subsection{Obtain Permutations by Swapping Adjacent and Concurrent Operations with Same Timestamp}
%\label{subsec:obtain permutations by swapping adjacent and concurrent operations with same timestamp}

%Similarly, when we deal with timestamp-order linearizations, we have a similar lemma of Lemma \ref{lemma:given two sequence consistent with visibility order, one can be obtained from the other}. The following lemma states that, given two specification sequences $(\alabelset_1, \aseqord_1)$ and $(\alabelset, \aseqord_2)$ that are generated from a same history and both consistent with visibility relation and the timestamp order, we can obtain $\aseqord_2$ from $\aseqord_1$ by several time of swapping adjacent pair of concurrent operations with same timestamp.

%\begin{lemma}
%\label{lemma:given two sequence consistent with visibility order and timestamp order, one can be obtained from the other}
%Given a history $(\alabelset,\avisord)$ and two two specification sequences $(\alabelset_1, \aseqord_1)$ and $(\alabelset, \aseqord_2)$ that are both consistent with $\avisord$ and the timestamp order. If $\aseqord_1 \neq \aseqord_2$, then we can obtain $\aseqord_2$ from $\aseqord_1$ by several time of swapping adjacent pair of concurrent operations with same timestamp.
%\end{lemma}

%\begin {proof}

%First, we need to prove that, if $\mathit{diff}(\aseqord_1,\aseqord_2) \neq \emptyset$, then, there exists $(\alabel_1,\alabel_2) \in \mathit{diff}(\aseqord_1,\aseqord_2)$, such that $\alabel_1$ and $\alabel_2$ are concurrent, $\alabel_1$ and $\alabel_2$ are adjacent in $\aseqord_1$, and $\alabel_1$ and $\alabel_2$ have a same ``real'' or ``virtual'' timestamp.

%We prove this by contradiction. Assume $\mathit{diff}(\aseqord_1,\aseqord_2) \neq \emptyset$, and for each $(\alabel_1,\alabel_2) \in \mathit{diff}(\aseqord_1,\aseqord_2)$, we have that either $\alabel_1$ and $\alabel_2$ are not concurrent, or $\alabel_1$ and $\alabel_2$ are not adjacent in $\aseqord_1$, or $\alabel_1$ and $\alabel_2$ have different timestamps.

%Similarly as Lemma \ref{lemma:given two sequence consistent with visibility order, one can be obtained from the other}, we can prove that, $\alabel_1$ and $\alabel_2$ are concurrent, and $\alabel_1$ and $\alabel_2$ are adjacent in $\aseqord_1$. Thus, the only possibility is that $\alabel_1$ and $\alabel_2$ have different timestamps, which contradicts the assumption that $\alabel_1$ and $\alabel_2$ are both consistent with the timestamp order. This completes the proof of the first part.

%Since $\aseqord_1 \neq \aseqord_2$, we have $\mathit{diff}(\aseqord_1,\aseqord_2) \neq \emptyset$, and then, as discussed above, there exists $(\alabel,\alabel') \in \mathit{diff}(\aseqord_1,\aseqord_2)$, such that $\alabel$ and $\alabel'$ are concurrent, $\alabel$ and $\alabel'$ are adjacent in $\aseqord_1$, and $\alabel_1$ and $\alabel_2$ have a same timestamp. Let $\aseqord_3 = \mathit{swap}(\aseqord_1,\alabel,\alabel')$. It is easy to see that $\mathit{diff}(\aseqord_1,\aseqord_2) > \mathit{diff}(\aseqord_3,\aseqord_2)$. Therefore, by several times of above process, we finally obtain $\aseqord_2$ from $\aseqord_1$ by swapping pairs of adjacent and concurrent operations. This completes the proof of this lemma. $\qed$
%\end {proof}
...
We prove by induction on executions. Obvious they hold in $\aglobalstate_0$. Assume they hold along the execution $\aglobalstate_0 \xrightarrow{}^* \aglobalstate$ and there is a new transition $\aglobalstate \xrightarrow{} \aglobalstate'$. We need to prove that they still hold in $\aglobalstate'$. We only need to consider when a replica do generator or effector of {\tt inc} and {\tt dec}:
    ...
    \item[-] For case when replica $\arep$ apply effector $(P_{\alabel},N_{\alabel})$ of a {\tt inc} operation $\alabel$: We only need to prove Annotation2. Let $S=(P,N)$ and $S'=(P',N')$ be the replica state of replica $\arep$ of $\aglobalstate$ and $\aglobalstate'$, respectively. Assume $\alabel$ happens on replica $\arep_{\alabel}$.
...
\item[-] For case when replica $\arep$ apply effector $(a,\ats_a)$ of an operation $\alabel = \alabelshort[{\tt write}]{a}$: We only need to prove Annotation2. Let $S=(x,\ats)$ and $S'=(x',\ats')$ be the replica state of replica $\arep$ of $\aglobalstate$ and $\aglobalstate'$, respectively.
...
Given a replica state $S$, assume we obtained $S'$ from $S$ by applying $(a_1,V_1)$ and then applying $(a_2,V_2)$, and assume we obtained $S''$ from $S$ by applying $(a_2,V_2)$ and then applying $(a_1,V_1)$. By the causal delivery assumption, for each operation $\alabel'$ that generate some effector $(a',V') \in S$, %$\alabelshort[{\tt write}]{a_1,V_1}$
...
We prove by induction on executions. Obvious they hold in $\aglobalstate_0$. Assume they hold along the execution $\aglobalstate_0 \xrightarrow{}^* \aglobalstate$ and there is a new transition $\aglobalstate \xrightarrow{} \aglobalstate'$. We need to prove that they still hold in $\aglobalstate'$. We only need to consider when a replica do generator or effector of {\tt write}:
    ...
    \item[-] For case when replica $\arep$ apply effector $(a,V')$ of a {\tt write} operation $\alabel$: We only need to prove Annotation2. Let $S$ and $S'$ be the replica state of replica $\arep$ of $\aglobalstate$ and $\aglobalstate'$, respectively.
...
We prove by induction on executions. Obvious they hold in $\aglobalstate_0$. Assume they hold along the execution $\aglobalstate_0 \xrightarrow{}^* \aglobalstate$ and there is a new transition $\aglobalstate \xrightarrow{} \aglobalstate'$. We need to prove that they still hold in $\aglobalstate'$. We only need to consider when a replica do generator or effector of {\tt write} or {\tt remove}:
    ...
    \item[-] For case when replica $\arep$ apply effector $(A_a,R_a)$ of an operation $\alabel = \alabelshort[{\tt write}]{a}$: We only need to prove Annotation2. Let $S=(A,R)$ and $S'=(A',R')$ be the replica state of replica $\arep$ of $\aglobalstate$ and $\aglobalstate'$, respectively.
...
\item[-] For case when replica $\arep$ apply effector $(A'',R'')$ of an operation $\alabel = \alabellongind[{\tt add}]{a}{}{\ats_a}{}$: We only need to prove Annotation2. Let $S=(A,R)$ and $S'=(A',R')$ be the replica state of replica $\arep$ of $\aglobalstate$ and $\aglobalstate'$, respectively.
...
\noindent {\bf Possibility $1$:} In $\alabelshort[{\tt integrateIns}]{w_1,w_b,w_2}$ of case $1$, $d_{min} > j_e$, while in $\alabelshort[{\tt integrateIns}]{w_1,w_b,w_2}$ of case $2$, $d_{min} = j_e$. Or we can say, $w_b$ is the only W-character that has degree $j_e$ and is between $w_1$ and $w_2$ in $\sigma_e$.
...
\item[-] Let us prove $\mathsf{Refinement}$:
    ...
    \item[-] Assume $\refmap(\sigma) = (l,T)$. If $\sigma'$ is obtained from $\sigma$ by applying an effector $\delta$ produced by an operation $\alabelshort[{\tt remove}]{a}$. By the {\textred{causal delivery}} assumption, we can see that a W-character $w_a$ of $a$ is already in $\sigma$, and then, $a \in l$. By the Wooki algorithm, we can see that $\sigma'$ is obtained from $\sigma$ by setting the flag of $w_a$ into $\mathit{false}$. Let $\refmap(\sigma) = (l',T')$. It is obvious that $l=l'$, and $T' = T \cup \{ a \}$. Thus, we have $\refmap(\sigma) \specarrow{\alabelshort[{\tt remove}]{a}} \refmap(\sigma')$.
...
\item[-] $parent \in N \times N$ represents the parent relation. $(node_1,node_2) \in parent$, if the T-identifiers of T-characters of $node_1$ is of the form $c_1 \cdot \ldots \cdot c_n \cdot (p,\_)$, one of the following cases holds:
    ...
    \item[-] The T-identifiers of T-characters of $node_2$ is of the form $c_1 \cdot \ldots \cdot c_n \cdot (p,id_p) \cdot (p',\_)$, while $c_1 \cdot \ldots \cdot c_n \cdot (p,id_p)$ is a T-identifier of T-characters $w_1$ of $node_1$. We say that $node2$ is a ``right son'' of $node_1$ as well as of $w_1$.  

    \item[-] There exists $node_0 \in N$, such that the T-identifier of T-characters of $node_0$ is $\epsilon$.
    \end{itemize}
\end{itemize}

According to the definition of T-tree, we can see that in the major-node-tree $(N,parent)$ of $tree$, each node can reach $node_0$ by several times of $parent$ relation, and the $parent$ relation is obviously acyclic.

The following lemma state that, the order $<_t$ of a T-tree is a total and acyclic order.

\begin{lemma}
\label{lemma:the <t order is a total and acyclic order}
Given a T-tree $tree$ and the order $<_t$ over T-identifiers of T-characters of $tree$, then, $<_t$ is a total and acyclic order.
\end{lemma}
...
\item[-] If $tid_1$ is not a descendant of $tid_1$ and $tid_1$ is not a descendant of $tid_2$. We can see that there exists $c''_1,\ldots,c''_k$, such that $tid_1 = c''_1 \cdot \ldots \cdot c''_k \cdot x_1 \cdot x_2 \cdot l_1$, $tid_2 = c''_1 \cdot \ldots \cdot c''_k \cdot y_1 \cdot y_2 \cdot l_2$, and $x_1 \neq y_1$. Then,
    ...
    \begin{itemize}
    \setlength{\itemsep}{0.5pt}
    \item[-] If $x_1 \in \{ 0, (0,\_) \}$ and $y_1 \in \{ 0, (0,\_) \}$, then $tid_1 <_t tid_2$.
...
\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] $w^l <_t w_{right}$, where $w_{left}$ (resp., $w_{right}$) is a W-character belongs to a node that is a descendant of the ``left son'' of $node$ (resp., the ``right son'' of $node$).

\item[-] $w^l <_t w^i$, where $w_{left}$ is a W-character that is a W-character belongs to a node that is a descendant of the ``left son'' of $node$, and $w^i$ is either $w_i$, or a W-character belongs to a node that is a ``descendent'' of $w_i$.

\item[-] Given $i<j$, we have $w^i <_t w^j$, where and $w^i$ resp., ($w^j$) is either $w_i$ (resp., $w_j$), or a W-character belongs to a node that is a ``descendent'' of $w_i$ (a ``descendent'' of $w_j$).

\item[-] $w^i <_t w^r$, where $w^i$ is either $w_i$, or a W-character belongs to a node that is a ``descendent'' of $w_i$, and $w_{right}$ is a W-character belongs to a node that is a descendant of the ``right son'' of $node$.
\end{itemize} 

Therefore, we can see that $<_t$ is a order for traverse the W-characters of the major-node-tree. For each node $node = w_1 \cdot \ldots \cdot w_n \in N$, we first traverse W-characters in its ``left son'' of $node$; then, for each $i$, we traverse $w_i$ and W-characters in nodes of its descendant; finally, we traverse W-characters in its ``right son'' of $node$. This process never goes back. Therefore, it is easy to see that $<_t$ is acyclic. This completes the proof of this lemma. $\qed$
\end {proof}
