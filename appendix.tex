...
Let us give the operation-based last-writer-win-element-set (LWW-element-set) of \cite{ShapiroPBZ11} in Listing~\ref{lst:LWW-element-set}. Each payload contains a data $x$ and the timestamp $ts$ of $x$. In applying effector, the local state will be updated if the timestamp of effector is larger than that of replica state. Here $x_0$ is a special initial data and $\ats_0$ is a special initial timestamp.
...
\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] Annotation2: Given a replica state $S = (x,\ats_x)$. Then, $\ats_x$ = $max(\{ \ats'' \vert \exists \alabel = \alabellongind[{\tt write}]{\_}{}{\ats''}{}$, the effector of $\alabel$ has already been applied in the local state $S \})$.
\end{itemize}
