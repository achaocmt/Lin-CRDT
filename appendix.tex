...
\item There exists a function $\gconfres: \labels^* \rightarrow \abstates$ (Here the name $\gconfres$ is short for global conflict resolution). Given sequence $s \in \labels^*$, $\gconfres(s)$ is defined as follows: assume $\abstate_0$ is the initial abstract state of \Spec{},
...
\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] $\mathsf{ConsistentChoice}$: Given two sub-sequences $s_1,s_2$, such that $s_1$ and $s_2$ are both consistent with $\avisord{}$, and $s_1$ and $s_2$ are both closed under $\avisord{}^{-1}$. Then, $\gconfres(s_1) \xrightarrow{ s_2 - s_1 }^* \gconfres(s_2)$, where $s_2 - s_1$ denotes the sequences obtained from $s_2$ by removing elements of $s_1$. 
...
Since $\alabellong[{\tt read}]{}{c \cdot b}{}$ and $\alabellong[{\tt read}]{}{a \cdot b \cdot c}{}$ see different operations, our convergence requirement is not enough to rule them out. Instead, the $\mathsf{ConsistentChoice}$ condition is able to rule them out. $\mathsf{ConsistentChoice}$ essentially represents that, a ``bigger state'' follows the same choice in a ``smaller'' state. 
\end{itemize}
...
\label{subsec:proof methodology for RA-linearizability w.r.t non-deterministic sequential specifications} 
...
We do the following constructions: 

\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] We introduce a function $\igconfres: \labels^* \rightarrow \states$ to explicitly record the consequence of applying downstreams. Given a sequence $s$ of dwonstream consistent with visibility relation, $\igconfres(s)$ is the local state obtained by applying downstreams of $s$ in the order of $s$. 

\item[-] For each local state $\igconfres(s)$, we generate a abstract state $\gconfres(s)$, and the refinement mapping $\refmap$ maps $\igconfres(s)$ into $\gconfres(s)$. Or we can say, $\refmap{}(\igconfres(s)) = \gconfres(s)$. 
\end{itemize}

$\igconfres(s)$ may be undefined for some sequence $s$, for example, apply the downstream of $\alabelshort[{\tt remove}]{a}$ when no downstream of inserting $a$ has been applied. Here the name $\igconfres$ is short for global conflict resolution in implementations.

Then, we need to prove the following properties: 

\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] We prove that any two downstreams that correspond to two ``concurrent'' operations commute. 

\item[-] We prove $\mathsf{Refinement}$ holds between $\igconfres(s)$ and $\gconfres(s)$ in a way as follows:  

\begin{enumerate}
\item If both $\igconfres(s)$ and $\igconfres(s \cdot \alabel)$ has definition, and both $s$ and $s \cdot \alabel$ are consistent with the visibility relation. Then, $\gconfres(s)\xRightarrow{\alabel}\gconfres(s \cdot \alabel)$, where $\xRightarrow{\alabel}$ is the transition function of $\Spec$, and 

\item if a query $\alabel$ is applied on a state $\igconfres(s)$ or it is introduced by a rewriting of a query-update that executes \lstinline|atSource| on a state $\igconfres(s)$, then $\refmap(\sigma)\xRightarrow{\alabel}\refmap(\sigma)$.
\end{enumerate}
\end{itemize}

From each local state $\igconfres(s)$, we generate a abstract state $\gconfres(s)$, and the refinement mapping $\refmap$ maps $\igconfres(s)$ into $\gconfres(s)$. Or we can say, $\refmap{}(\igconfres(s)) = \gconfres(s)$.


In our proof of  Similar as in Section \ref{sec:proofs}, we prove that any two downstreams that correspond to two ``concurrent'' operations commute.
