...
captionpos=b,label={lst:operation-based multi-value register}]
  ...
  write(a) :
    atSource :
      ...
      //@ let lin' = lin@|$\,\cdot\,\alabellongind[{\tt readIds}]{}{S}{}\,\cdot\,\alabelshort[{\tt write}]{a,V',S}$|@
    downStream(a, V') :
  ...
  read() :
    ...
    //@ let lin' = lin@|$\,\cdot\,\alabellongind[{\tt read}]{}{S_1}{}$|@
    return @|$S_1$|@
\end{lstlisting}
\end{figure}
...
\cite{ShapiroPBZ11} gives a operation-based counter implementation, which is shown in Listing~\ref{lst:operation-based multi-value register}. A payload is a integer $ctr$. 
...
captionpos=b,label={lst:operation-based counter}]
  ...
  inc() :
    atSource :
      //@ let lin' = lin@|$\,\cdot\,\alabelshort[{\tt inc}]{} $|@
    downStream(inc) :
      ctr = ctr + 1 
  
  dec() :
    atSource :
      //@ let lin' = lin@|$\,\cdot\,\alabelshort[{\tt dec}]{} $|@
    downStream(dec) :
      ctr = ctr - 1 

  read() : 
    //@ let lin' = lin@|$\,\cdot\,\alabellong[{\tt read}]{}{ ctr }{}$|@
    return ctr 
\end{lstlisting}
\end{figure}
...
In local state of each replica, Wooki algorithm stores the list as a sequence of W-characters. A W-character $c$ is a tuple $(id,v,degree,flag)$, where $id$ is the identifier of $c$; $v$ is the value of $c$; $degree$ is the degree of $c$; $flag \in \{ \mathit{true},\mathit{false} \}$ is the flag of $c$ and indicates whether $c$ is ``visible'' in list. A identifier $id$ of W-character is a unique timestamp. %tuple $(ctr,\arep)$, where $ctr \in \mathbb{N}$.
We use $degree(c)$ to denote the degree of $c$.

A W-string is an ordered sequence of W-characters $c_{begin} \cdot c_1 \cdot \ldots \cdot c_n \cdot c_{end}$, where $c_{begin}$ and $c_{end}$ are special W-characters that mark the beginning and the ending of the sequence. The values of $c_{begin}$ and $c_{end}$ are $\circ_{begin}$ and $\circ_{end}$, respectively. The degree of $c_{begin}$ and $c_{end}$ are $0$. We define the following function for a W-string $str$:
...
\item[-] $pos(str,c)$ returns the position of W-character $c$ in $S$.

\item[-] $insert(str,c,p)$ inserts W-character $c$ into $str$ at position $p$.

\item[-] $subseq(str,c_1,c_2)$ returns the part of $str$ between the W-characters $c_1$ and $c_2$ (excluding $c_1$ and $c_2$).
...
A total order $<_{id}$ is given for identifiers of W-characters for conflict resolution ($<_{id}$ is just the total order of timestamp). Given a W-string $str$ and two W-characters $c_1,c_2$ of $str$, we write $c_1 <_{str} c_2$ to indicate that $pos(str,c_1) < pos(str,c_2)$.
...
captionpos=b,label={lst:wooki algorithm}]
  ...
  addBetween(a,b,c) :
    ...
    downStream((c,@|$c_p$|@,@|$c_n$|@)) : with @|$c = (ts, a, max(degree(c_p),degree(c_n)) +1, \mathit{true})$|@
...
To do $\alabelshort[{\tt addBetween}]{a,b,c}$, we first ensure that $a$ and $c$ are adjacent in $string_s$ and $b$ is not in $string_s$. Then, we generate a W-character $c$ for value $b$, and calls method $integrateIns(c,c_p,c_n)$ to put $c$ between $c_p$ and $c_n$, which are the W-characters of $a$ and $c$ in $string_s$, respectively.

$integrateIns(c,c_p,c_n)$ is a recursive method and works as follows: If there are no W-character between $c_p$ and $c_n$ (for example, in the current replica), then $c$ is put after $c_p$. Else, Wooki selects a sequence $F$ of W-characters, such that each W-character of $F$ is between $c_p$ and $c_n$, and has minimal degree. It can be proved that W-characters in $F$ are sorted by the $<_{id}$ order. Then, we choose the position of $c$, and recursive call $integrateIns(c,F[i-1],F[i])$. We can see that, the minimal degree of the sub-sequence of $S$ between $F[i-1]$ and $F[i]$ is larger than that of the sub-sequence of $S$ between $c_p$ and $c_n$.
...
Let us go back to \figurename~\ref{fig:a wrong history w.r.t listbets}. Let $\sigma_1$ be the local state of $\arep_2$ after we do $\alabelshort[{\tt addBetween}]{\circ_{begin},c,\circ_{end}}$, and let $\sigma_2$ be the local state of $\arep_2$ after we apply the downstream of $\alabelshort[{\tt addBetween}]{\circ_{begin},a,\circ_{end}}$.
