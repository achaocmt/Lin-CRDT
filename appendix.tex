...
We use $degree(w)$ to denote the degree of $w$.

A W-string is an ordered sequence of W-characters $w_{begin} \cdot w_1 \cdot \ldots \cdot w_n \cdot w_{end}$, where $w_{begin}$ and $w_{end}$ are special W-characters that mark the beginning and the ending of the sequence. The values of $w_b$ and $w_e$ are $\circ_b$ and $\circ_e$, respectively. The degree of $w_{begin}$ and $w_{end}$ are $0$. We define the following function for a W-string $str$:
...
A total order $<_{id}$ is given for identifiers of W-characters for conflict resolution ($<_{id}$ is just the total order of timestamp). Given a W-string $str$ and two W-characters $a,b$ of $str$, we write $a <_{str} b$ to indicate that $pos(str,a) < pos(str,b)$.
...
captionpos=b,label={lst:wooki algorithm}]
  ...
  addBetween(b,a,c) :
    atSource :
      ...
      //@ let lin@|$'$|@ = lin@|$\,\cdot\,\alabelshort[{\tt addBetween}]{b,a,c}$|@
    downStream((w,@|$c_p$|@,@|$c_n$|@)) : with @|$w = (ts, a, max(degree(c_p),degree(c_n)) +1, \mathit{true})$|@
      integrateIns(@|$c_p,w,c_n$|@)
...
The tree-doc algorithm of \cite{DBLP:conf/icdcs/PreguicaMSL09,DBLP:journals/corr/abs-0710-1784} is given in Listing~\ref{lst:Tree-Doc algorithm}.

In local state of each replica, tree-doc algorithm stores the list as a set of T-characters. A T-character $w$ is a tuple $(v,tid,g)$, where $v$ and $g$ are the value of $w$, and $tid$ is a T-identifier of $w$.

The requirement of T-identifier are as follows: there should be a total order $<_t$ over the T-identifiers, and the T-identifiers are designed to satisfy a ``dense'' property: for each T-identifier $tid_1,tid_2$, assume $tid_1 <_t tid_2$, then, there must exists a T-identifier $tid_3$, such that $tid_1 <_t tid_2 <_t tid_3$.
...
For example, \figurename~\ref{fig:an execution of tree-doc, the local state of replica r1 after execution, and the T-identifiers of T-characters} shows an execution of tree-doc. Let $w_a,\ldots,w_h$ be the W-character of $a,\ldots,h$. We can see that both $w_b$ and $w_c$ are inserted as ``left son'' of $w_a$. Therefore, the set of T-characters of $w_a$ and $w_c$ are a major node and we use a box containing $w_b$ and $w_c$ to emphasize a major node. Similarly, $w_d$ and $w_e$, $w_f$ and $w_g$ also forms a major node.

A T-identifier $tid$ of a T-character $w$ is a sequence $tid = c_1 \cdot \ldots \cdot c_n \cdot (p,id)$, where $id$ is a unique identifier, and each for each $i$, $c_i \in \{ 0,1,(1,id') \vert id'$ is an identifier $\}$. We require such $id$ to be unique, and with a total order $<$. When the context is clear, we will call $id$ the identifier of $w$. Each $c_i$ of $tid$ represent how to choose the next node in the $i$-th step of the ``path''. If $c_i=0$ (resp., $c_i=1$), then the next node is a node as ``left son'' (resp., ``right son'') of the current major node; Else, $c_i = (1,id')$, and this represents that the next node is chosen from ``sons'' of a minor node with identifier $id'$. For example, in \figurename~\ref{fig:an execution of tree-doc, the local state of replica r1 after execution, and the T-identifiers of T-characters}, the T-identifier of $w_h$ is $0 \cdot (1,id_h)$, which represent that $w_h$ is a ``right son'' of the major node of $w_b$ and $w_c$; the T-identifier of $w_g$ is $(0,id_b) \cdot (1,id_g)$, which represent that $w_g$ is not a ``right son'' of the major node of $w_b$ and $w_c$, but a ``right son'' of only minor node $w_b$. Note that, as in \figurename~\ref{fig:an execution of tree-doc, the local state of replica r1 after execution, and the T-identifiers of T-characters}, the corresponding tree is not a binary tree.

The path to the root is fixed to be $\epsilon$, and therefore, the identifier of $w_a$ in \figurename~\ref{fig:an execution of tree-doc, the local state of replica r1 after execution, and the T-identifiers of T-characters} is $(\epsilon,id_a)$.

We say a set $T$ of T-characters is a legal tree of tree-doc, if:

\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] There exists a T-character of $T$ with T-identifier $(\epsilon,\_)$.

\item[-] Each value of T-character is unique, and each T-character has a unique identifier.

\item[-] For each T-character $w=(a,tid) \in T$, assume $tid = c_1 \cdot \ldots \cdot c_n \cdot (p,id)$. Then,for each $i$,

    \begin{itemize}
    \setlength{\itemsep}{0.5pt}
    \item[-] If $c_i = p_i \in \{ 0,1 \}$, then there exists a T-character $w' = (\_, c_1 \cdot \ldots \cdot c_{i-1} \cdot (p_i,\_) ) \in T$,

    \item[-] Else, if $c_i = (p_i,id_i)$, then there exists a T-character $w' = (\_, c_1 \cdot \ldots \cdot c_{i-1} \cdot (p_i,id_i) ) \in T$, and there exists another T-character $w'' = (\_, c_1 \cdot \ldots \cdot c_{i-1} \cdot (p_i,id'_i) ) \in T$ and $id < id'$. Or we can say, the position of $w'$ is a major node containing $w'$ and $w''$, and the identifier of $w'$ is less that that of $w''$.
    \end{itemize}
\end{itemize}

Given a legal tree of tree-doc,



\figurename~\ref{fig:an execution of tree-doc, the local state of replica r1 after execution, and the T-identifiers of T-characters}.
...
\item[-] $\abstate \xRightarrow{\alabellong[{\tt read}]{}{ S }{}} \abstate$, where $S = \{ a \vert \exists id, (a,id) \in \abstate \}$.
\end{itemize}
...
Each abstract state $\abstate = (l,T)$ contains a sequence $l$ and a set $T$. The element $l$ is the list of all input values, whether already removed or not; while $T$ stores the removed values and is used as tombstone. The sequential specification $\mathit{listBet}_s$ of list with add-between interface is given with the transitions as follows:
\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] $\big((a_1 \cdot \ldots \cdot a_n,T)\ |\ 1 \leq i \leq k <j \leq n \wedge a \ is \ unique \big) \xrightharpoonup{ \alabelshort[{\tt addAfter}]{a_i,a,a_j} } (a_1 \cdot \ldots \cdot a_k \cdot a \cdot a_{k+1} \cdot \ldots \cdot a_n,T)$
\item[-] $\big((a_1\cdot \ldots \cdot a_n,T)\ |\ 1 \leq k \leq n \big) \xrightharpoonup{ \alabelshort[{\tt remove}]{a_k} } (l,T \cup \{ a_k \})$
\item[-] $(l,T) \xrightharpoonup{\alabellong[{\tt read}]{}{s}{}} (l,T)$, here $s$ is obtained from $l$ by removing values in $T$
\end{itemize}

Method $\alabelshort[{\tt addBetween}]{a_k,a}$ puts $a$ into a random position between $a_i$ and $a_j$ in $l$, and we assume that each value is put into list at most once. $\alabelshort[{\tt remove}]{a_k}$ adds $a_k$ into $T$. $\alabellong[{\tt read}]{}{s}{}$ returns the list content not in $T$.

Assume that the initial value of list is $(\circ_{begin} \cdot \circ_{end},\emptyset)$, and $\circ_{begin}$ and $\circ_{end}$ are never removed. When the context is clear, in $\ensuremath{\tt read}$ operation, we will omit $\circ_{begin}$ and $\circ_{end}$ in return value.
...
Each abstract state $\abstate$ is a set of tuples $(a,id)$, where $a$ is a data and $id$ is a identifier. The sequential specification $\mathit{OR-Set}_s$ of OR-Set is given by the transitions:
\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] $\abstate \xrightharpoonup{\alabellong[{\tt readIds}]{a}{ S }{} } \abstate$, where $S = \{ (a,id) \vert (a,id) \in \abstate \}$

\item[-] $\abstate \xrightharpoonup{ \alabelshort[{\tt remove}]{a,S} }  \abstate \setminus S$

\item[-] $\big(\abstate\ |\ \mathtt{id}\ \text{does not occur in } \abstate\big) \xrightharpoonup{ \alabelshort[{\tt add}]{a,id} }  \abstate \cup \{ (a,\mathtt{id}) \}$

\item[-] $\abstate \xrightharpoonup{\alabellong[{\tt read}]{a}{ S }{}} \abstate$, where $S = \{ a \vert \exists \mathtt{id}, (a,\mathtt{id}) \in \abstate \}$
\end{itemize}
...
Each abstract state $\abstate = (l,T)$ contains a sequence $l$ and a set $T$. The element $l$ is the list of all input values, whether already removed or not; while $T$ stores the removed values and is used as tombstone. The sequential specification {$\mathit{listAft}_s$} of list with add-after interface is given with the transitions as follows:
\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] $\big((a_1 \cdot \ldots \cdot a_n,T)\ |\ k \leq n \wedge a \ is \ unique \big) \xrightharpoonup{ \alabelshort[{\tt addAfter}]{a_k,a} } (a_1 \cdot \ldots \cdot a_k \cdot a \cdot a_{k+1} \cdot \ldots \cdot a_n,T)$
\item[-] $\big((a_1\cdot \ldots \cdot a_n,T)\ |\ 1 \leq k \leq n \big) \xrightharpoonup{ \alabelshort[{\tt remove}]{a_k} } (l,T \cup \{ a_k \})$
\item[-] $(l,T) \xrightharpoonup{\alabellong[{\tt read}]{}{s}{}} (l,T)$, here $s$ is obtained from $l$ by removing values in $T$
\end{itemize}

Method $\alabelshort[{\tt addAfter}]{a_k,a}$ puts $a$ immediately after $a_k$ in $l$, and we assume that each value is put into list at most once. $\alabelshort[{\tt remove}]{a_k}$ adds $a_k$ into $T$. $\alabellong[{\tt read}]{}{s}{}$ returns the list content not in $T$.
...
Given a a W-string $s$ and two W-characters $c_1,c_2$, we say that $c_1$ and $c_2$ are degree-$i$-adjacent in $s$, if there exists W-characters $c_3,c_4$, such that $c_3$ and $c_4$ are with degree $i-1$, there are no W-character with degree $i-1$ between $c_3$ and $c_4$ in $s$, $c_1$ and $c_2$ are with degree $i$, there are no W-character with degree $i$ between $c_1$ and $c_2$ in $s$, and we have $c_3 <_s \{ c_1,c_2 \} <_s c_4$. The following lemma confirms an intuition of Wooki algorithm: in recursive method $integrateIns$, the W-characters of W-string $F$ are sorted by $<_{id}$ order.
...
It is obvious that this property holds initially. Let us prove the induction part by contradiction. Assume this property hold for W-string $s$, let $s'$ be the W-string obtained from $s$ by doing $\alabelshort[{\tt addBetween}]{a,b,c}$, and this property does not hold for $s'$. $c_b$ be the W-character of $b$. Assume the degree of $c_b$ is $j$.

Obviously, the W-characters that are not degree-$j$-adjacent with $c_b$ are still the same in $s'$ as that in $s$.

Assume three exists W-character $c_3$ of $s$, such that $c_3$ and $c_b$ are degree-$j$-adjacent, $c_3 <_{s'} c_b$, and $c_b <_{s'} c_3$. Then, there exists W-character $c_1,c_2$, such that $c_1$ and $c_2$ are with degree $j-1$, there are no W-character with degree $j-1$ between $c_1$ and $c_2$ in $s'$, and $c_1 <_{s'} c_b <_{s'} c_3 <_{s'} c_2$.

Note that $c_b$ and $c_3$ are between $c_1$ and $c_2$ in $s'$, $c_1$ is the rightmost W-character with degree $j-1$ that are before $c_b$ and $c_3$ in $s'$, and $c_2$ is the leftmost W-character with degree $j-1$ that are after $c_b$ and $c_3$ in $s'$. According to Wooki algorithm, in the process of $\alabelshort[{\tt addBetween}]{a,b,c}$, the recursive method $\alabelshort[{\tt integrateIns}]{c_1,c_2,c_b}$ will be called and then, $c_3$ and $c_b$ are sorted according to $<_{id}$ order, contradicts the assumption that $c_3 <_{s'} c_b$ and $c_b <_{s'} c_3$. Therefore, such $c_3$ does not exists in $s'$. This completes the proof of this lemma. $\qed$
\end {proof}


The following lemma states that, the order of $\{$values of $\sigma \} \cup \{ b \}$ in the following two cases are the same: either we insert $b$ into $\sigma$, or first insert another item $e$ and then insert $b$. Or we can say, this lemma states that, inserting a value is ``independent'' from inserting another value.
...
Assume from a local state $\sigma$, we obtain $\sigma_b$ by applying the downstream of $\alabelshort[{\tt addBetween}]{a,b,c}$, and obtain $\sigma_{eb}$ by first applying the downstream of $\alabelshort[{\tt addBetween}]{d,e,f}$ and then applying the downstream of $\alabelshort[{\tt addBetween}]{a,b,c}$. Then, the order of $\{$values of $\sigma \} \cup \{ b \}$ is the same for $\sigma_b$ and $\sigma_{eb}$.
\end{lemma}

\begin {proof}
Let $c_a,c_b,c_c,c_d,c_e,c_f$ be the W-character of $a,b,c,d,e,f$, respectively. Assume $\sigma_e$ is obtained from $\sigma$ by applying the downstream of $\alabelshort[{\tt addBetween}]{d,e,f}$. Let us use case $1$ to mention the process of $\alabelshort[{\tt integrateIns}]{c_a,c_b,c_c}$ from $\sigma$. Let us use case $2$ to mention the process of $\alabelshort[{\tt integrateIns}]{c_a,c_b,c_c}$ from $\sigma_e$. Let $j_e$ be the degree of $c_e$.

We need to prove that, for each degree $i$, the order of $\{$values of degree $i$ in $\sigma \} \cup \{ b \}$ is the same for case $1$ and case $2$. We prove this by considering all possible degrees.

\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] It is obvious that, for degree $i < j_e$, case $1$ and case $2$ both calls $\alabelshort[{\tt integrateIns}]{c_1,c_b,c_2}$ for some $c_1$ and $c_2$, and they work in the same way.

\item[-] For degree $j_e$, case $1$ and case $2$ both calls $\alabelshort[{\tt integrateIns}]{c_1,c_b,c_2}$ for some $c_1$ and $c_2$.

    Let $c'_1,\ldots,c'_n$ be the W-characters with degree $j_e +1$ between $c_1$ and $c_2$ in $\sigma$. Assume that $c'_1 <_{\sigma} \ldots <_{\sigma} c'_n$. By Lemma \ref{lemma:in F of Wooki, W-characters are ordered in id-order}, we know that, $c'_1 <_{id} \ldots <_{id} c'_n$. Assume $c'_k <_{\sigma_e} c_e <_{\sigma_e} c'_{k+1}$. By Wooki algorithm, in inserting $c_e$, we will call $\alabelshort[{\tt integrateIns}]{c_1,c_e,c_2}$, and then, $c'_k <_{id} c_e <_{id} c'_{k+1}$. Thus, we have $c'_1 <_{id} \ldots <_{id} c'_k <_{id} c_e <_{id} c'_{k+1} <_{id} \ldots <_{id} c'_n$ and $c'_1 <_{\sigma_e} \ldots <_{\sigma_e} c'_k <_{\sigma_e} c_e <_{\sigma_e} c'_{k+1} <_{\sigma_e} \ldots <_{\sigma_e} c'_n$.

    If $c_b <_{id} c'_k$ or $c'_{k+1} <_{id} c_b$, assume $c'_m <_{id} c_b <_{id} c'_{m+1}$, then both case $1$ and case $2$ recursively calls $\alabelshort[{\tt integrateIns}]{c'_m,c_b,c'_{m+1}}$, and $\{c'_1,\ldots,c'_n,c_b\}$ is ordered by $<_{id}$ order. Moreover, from now on, case $2$ will work in the same way as case $1$.

    Else, we know that $c'_k <_{id} c_b <_{id} c'_{k+1}$. Assume that $c'_k <_{id} c_b <_{id} c_e <_{id} c'_{k+1}$. Then, case $1$ recursively calls $\alabelshort[{\tt integrateIns}]{c'_k,c_b,c'_{k+1}}$, and case $2$ recursively calls $\alabelshort[{\tt integrateIns}]{c'_k,c_b,c_e}$. We can see in both case $1$ and case $2$, $\{c'_1,\ldots,c'_n,c_b\}$ is ordered by $<_{id}$ order. The situation of $c'_k <_{id} c_2 <_{id} c_b <_{id} c'_{k+1}$ can be similarly proved.

    We can see that, if case $2$ recursively calls $\alabelshort[{\tt integrateIns}]{c'_k,c_b,c_e}$, then we have $c_b <_{id} c_e$; if case $2$ recursively calls $\alabelshort[{\tt integrateIns}]{c_e,c_b,c'_{k+1}}$, then we have $c_e <_{id} c_b$.

\item[-] Inductively, for a degree $k > j_e$, assume that case $1$ recursively calls $\alabelshort[{\tt integrateIns}]{c_1,c_b,c_2}$, and case $2$ recursively calls $\alabelshort[{\tt integrateIns}]{c_1,c_b,c_e}$. Let $c'_1,\ldots,c'_n$ be the W-characters with degree $k+1$ between $c_1$ and $c_e$ in $\sigma_e$. Assume that $c'_1 <_{\sigma_e} \ldots <_{\sigma_e} c'_n$. Let $c'_{m+1},\ldots,c'_n$ be the W-characters with degree $k+1$ between $c_e$ and $c_2$ in $\sigma_e$. Assume that $c'_1 <_{\sigma_e} \ldots <_{\sigma_e} c'_n$.

    By Lemma \ref{lemma:in F of Wooki, W-characters are ordered in id-order}, we can see that $c'_1 <_{id} \ldots <_{id} c'_n$. By Wooki algorithm, in inserting $c_e$, we will call $\alabelshort[{\tt integrateIns}]{c_1,c_e,c_2}$, and then, we can see that $c'_m <_{id} c_e <_{id} c'_{m+1}$.

    If $c_b <_{id} c'_m$, assume $c'_l <_{id} c_b <_{id} c'_{l+1}$, then both case $1$ and case $2$ recursively calls $\alabelshort[{\tt integrateIns}]{c'_l,c_b,c'_{l+1}}$, and $\{c'_1,\ldots,c'_n,c_b\}$ is ordered by $<_{id}$ order. Moreover, from now on, case $2$ will work in the same way as case $1$.

    Else, we know that $c'_m <_{id} c_b$. Since case $2$ recursively calls $\alabelshort[{\tt integrateIns}]{c_1,c_b,c_e}$, we know that $c_b <_{id} c_e <_{id} c'_{m+1}$. Then, case $1$ recursively calls $\alabelshort[{\tt integrateIns}]{c'_m,c_b,c'_{m+1}}$, and case $2$ recursively calls $\alabelshort[{\tt integrateIns}]{c'_m,c_b,c_e}$. We can see in both case $1$ and case $2$, $\{c'_1,\ldots,c'_n,c_b\}$ is ordered by $<_{id}$ order.
\end{itemize}
...
Therefore, the only possibility is that in $\sigma_{eb}$, $c_b$ and $c_e$ are adjacent, and there are no W-character between $c_b$ and $c_e$. Let $c_1$ be the right-most W-character that are before $c_b$ in $\sigma_{eb}$. According to Wooki algorithm, in the process of applying the downstream of $\alabelshort[{\tt addBetween}]{a,b,c}$ to $\sigma_e$, the last time of calling recursive method ${\tt integrateIns}$ must be $\alabelshort[{\tt integrateIns}]{c_1,c_b,c_e}$. Otherwise, since there is no W-character (except $c_b$) between $c_1$ and $c_e$, we can see that $c_b$ is not between $c_1$ and $c_e$ in $\sigma_{eb}$. Similarly as the proof of Lemma \ref{lemma:in Wooki algorithm,the order of sigma and b is the same, between insert b and first insert e and then insert b}, we can see that $c_b <_{id} c_e$.

For the case of $\sigma_{be}$, similarly, we can prove that $c_e <_{id} c_b$. This implies that $c_b <_{id} c_e \wedge c_e <_{id} c_b$, which is a contradiction. $\qed$
\end {proof}
