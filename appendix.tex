...
\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] $\alabelshort[{\tt addAt}]{a,k}$: Inserts value $a$ into position $k$ of the replica state. For $k$ exceeding the list size of replica state, $k$ will be inserted at the end of the list.
...
\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] Let $s$ be the list content in replica state. If $s = \epsilon$, then let $b = \circ$; else, if $\vert s \vert \geq k$, then let $b=s[k-1]$; else, let $b=s[\vert s \vert -1]$.

\item[-] Work as $\alabelshort[{\tt addAfter}]{b,a}$. Especially, the effector of $\alabelshort[{\tt addAt}]{a,k}$ is the effector of $\alabelshort[{\tt addAfter}]{b,a}$. 
\end{itemize}
...
\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] For {\tt addAt}: Given argument $a$ and $k$ of {\tt addAt}. Let $s$ be the list content in replica state. If $s = \epsilon$, then let $b = \circ$; else, if $\vert s \vert \geq k$, then let $b=s[k-1]$; else, let $b=s[\vert s \vert -1]$. 
...
Concerning the proof of $\mathsf{Refinement}$, we use the same refinement mapping $\refmap$ as in \sectionautorefname \ref{subsec:time-stamp order as linearizabtion}: $\refmap(N,Tomb) = (l,T)$, where $l={\tt traverse(N, Tomb)}$, and $T={\tt Tomb}$.
