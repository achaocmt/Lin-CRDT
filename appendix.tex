...
\item[-] Let us prove that $\mathsf{Refinement}$ holds. We consider a refinement mapping $\refmap$ defined as the identity function.
    ...
    Given $(b,V) \in S$ and assume that operation $\alabel'$ generates $(b,V)$. Since $\alinord$ is concistent with the visibility realtion and $fact2$, we can see that, $(\alabel,\alabel') \notin \avisord$. By $fact1$, we can see that, $\neg (V' < V)$. Therefore, we can see that $S' = S \setminus S_2 \cup \{ (a,V') \}$, where $S_2 = \{ (b,V) \vert (b,V) \in S \wedge V < V' \}$.
    ...
    Let us prove $S' = \abstate'$ by contradiction.
        ...
        Let $\alabel_c$ be the operation that generates effector $(c,V'')$. Since $(c,V'') \in S_2$, by $fact1$, we can see that, $(\alabel_c,\alabel) \in \avisord$. Since $(c,V'') \notin S_1$, we know that there exists a {\tt write} operation $\alabel_1$, such that $(\alabel_c,\alabel_1)$, $(\alabel_1,\alabel) \in \avisord$. Assume the effector of $\alabel_1$ is $(a_1,V_1)$. By $fact1$, we can see that, $V'' < V_1$. Since $(c,V'') \in S$, we can see that the effector of $\alabel_1$ has not been applied yet in $S$, and also the effector of $\alabel_1$ has not been applied yet in $S'$, while in $S'$, the effector of $\alabel$ has been applied. Since $(\alabel_1,\alabel) \in \avisord$, we can see that this violates the causal delivery assumption.
...
The following lemma states that the operation-based multi-value register is \crdtlinearizable{} w.r.t. $\specMVReg$. Lemma \ref{lemma:multi-value register is correct}. 
