...
The TreeDoc algorithm of \cite{DBLP:conf/icdcs/PreguicaMSL09,DBLP:journals/corr/abs-0710-1784} is given in Listing~\ref{lst:Tree-Doc algorithm}. 
...
Assume that the initial value of list is $(\circ_b \cdot \circ_e,\emptyset)$, and $\circ_b$ and $\circ_e$ are never removed. When the context is clear, in $\ensuremath{\tt read}$ operation, we will omit $\circ_b$ and $\circ_e$ in return value.
