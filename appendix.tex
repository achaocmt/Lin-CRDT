...
\item[-] $\big(\abstate\ |\ id \ is \ unique \big) \xrightarrow{ \alabellong[\mathsf{write}]{a,id,S}{} }  \abstate \setminus S \cup \{ (a,id) \}$
...
The query-update rewriting of OR-set is as follows: $\gamma( \alabellong[\mathsf{remove}]{a}{} ) = ( \alabellong[\mathsf{readIds}]{a}{S}, \alabellong[\mathsf{remove}]{a,S}{})$.
...
\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] $\abstate \xrightarrow{\alabellong[\mathsf{readIds}]{a}{ \{ (a,id) \vert (a,id) \in \abstate \} }} \abstate$

\item[-] $\abstate \xrightarrow{ \alabellong[\mathsf{remove}]{a,S}{} }  \abstate \setminus S$

\item[-] $\big(\abstate\ |\ id \ is \ unique \big) \xrightarrow{ \alabellong[\mathsf{add}]{a,id}{} }  \abstate \cup \{ (a,id) \}$

\item[-] $\abstate \xrightarrow{\alabellong[\mathsf{read}]{}{ \{ a \vert \exists id, (a,id) \in \abstate \} }} \abstate$
\end{itemize}

Here $readIds(a)$ returns the set of pairs with data $a$, $\alabellong[\mathsf{remove}]{a,S}{}$ removes $S$ from the abstract state, $add(a,id)$ puts $\{ (a,id) \}$ into the abstract state, and $read$ returns the value of or-set.
