...
\begin {proof} 
Given a distributed linearizable history $h$ and assume that $\mathit{lin} = (\ell_1,\mathit{id}_1,S_1) \cdot \ldots \cdot (\ell_k,\mathit{id}_k,S_k)$ is a linearization. We need to prove that, each such sequence $\mathit{lin}'$ is also a linearization of $h$
...
We prove this by showing that each such $\mathit{lin}'$ can be obtained from $\mathit{lin}$ by several times of swapping a pair of adjacent elements. 
