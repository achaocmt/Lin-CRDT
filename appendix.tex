...
Let us explain Definition \ref{definition:ralinearizability1 with non-deterministic specifications} from two aspects: Although $\Spec{}$ is non-deterministic, we use $\gconfres(s)$ to remember our choice for operation sequence $s$, and
...
To get rid of such histories, we use the condition ``Given two sub-sequences $s_1,s_2$ of $\aseqord\!\downarrow_{\updates}$ ... $\gconfres(s_1) \xrightarrow{ s_2 - s_1 }^* \gconfres(s_2)$'', which represents that, a ``bigger state'' follows the same choice in a ``smaller'' state.
\end{itemize}
...
\begin{itemize}
\item[-] We introduce a function $\igconfres: \labels^* \rightarrow \states$ to explicitly record the consequence of applying downstreams. Given a sequence $s$ of dwonstream, $\igconfres(s)$ is the local state obtained by applying downstreams of $s$ in the order of $s$. Here we assume that $s$ is a order consistent with visiblity relation. 

\item[-] From each local state $\igconfres(s)$, we generate a abstract state $\gconfres(s)$, and the refinement mapping $\refmap$ maps $\igconfres(s)$ into $\gconfres(s)$. Or we can say, $\refmap{}(\igconfres(s)) = \gconfres(s)$.
\end{itemize}
