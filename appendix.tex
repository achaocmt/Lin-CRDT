...
Therefore, we can see that $<_t$ is a order for traverse the W-characters of the major-node-tree. For each node $node = w_1 \cdot \ldots \cdot w_n \in N$, we first traverse W-characters in its ``left son'' of $node$ and its descendant; then, for each $i$, we traverse $w_i$ and W-characters in nodes of its descendant; finally, we traverse W-characters in its ``right son'' of $node$ and its descendant. This process never goes back. Therefore, it is easy to see that $<_t$ is acyclic. This completes the proof of this lemma. $\qed$
\end {proof}
...
Given an effector $w_b = (b,tid_b,b)$ of an operation $\alabel = \alabelshort[{\tt addBetween}]{a,b,c}$, assume $\alabel$ calls $\alabelshort[{\tt newID}]{tid_a,tid_c,id}$ to generate effector $w_b$. Given a T-tree $tree$, assume that $(\_,tid_a,a),(\_,tid_c,c) \in tree$, no T-character of $tree$ has ghost value $b$, no T-character of $tree$ has disambiguator $id$. Then, $tree \cup \{ w_b \}$ is a T-tree.
\end{lemma}
...
\begin{lemma}
\label{lemma:when we do addBetween(a,b,c) and generate an effector (b,tid_b,b), then tid_b is between T-identifiers of T-characters of a and c} 
...
\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] If $tid$ is a descendant of $tid_a$: Then, $tid$ is either a ``right son'' of $tid_a$, or a descendant of a ``right son'' of $tid_a$. We can see that $tid_2$ does not have ``left son''. 
    
    Assume that $tid = l_2 \cdot (p_2,id_2)$, then, $tid'' = l_2 \cdot p_2 \cdot (0,id)$. We can see that $tid''$ is a descendant of a ``right son'' of $tid_a$, and thus, $tid_a <_t tid''$. Since $tid''$ is a ``left son'' of $tid$, we can see that $tid'' <_t tid$.

\item[-] If $tid_a$ is a descendant of $tid$: Then, $tid_a$ is either a ``left son'' of $tid$, or a descendant of a ``left son'' of $tid$. We can see that $tid_a$ does not have ``right son''. 
    
    Assume that $tid_a = l_1 \cdot (p_1,id_1)$, then, $tid'' = l_1 \cdot p_1 \cdot (1,id)$. We can see that $tid''$ is a descendant of a ``left son'' of $tid$, and thus, $tid'' <_t tid$. Since $tid''$ is a ``right son'' of $tid_a$, we can see that $tid_a <_t tid''$. 
...
\item[-] Otherwise, there are two possibilities: 
    ...
    Assume that $tid_a = l_1 \cdot (p_1,id_1)$, then, $tid'' = l_1 \cdot p_1 \cdot (1,id)$. We can see that $tid''$ is a ``right son'' of $tid_a$, and thus, $tid_a <_t tid''$. Since $id_0 < id_2$, we can see that $tid'' <_t tid$. 
    \end{itemize}
...
Then, let us prove that tree-doc is \crdtlinearizable{} w.r.t $\mathit{listBet}_s$.

\begin{lemma}
\label{lemma:tree-doc is correct}
Tree-doc is \crdtlinearizable{} w.r.t $\mathit{listBet}_s$.
\end{lemma}

\begin {proof}
By the causal delivery assumption and Lemma \ref{lemma:if we add a T-character (an effector) into a T-tree, then we obtain another T-tree}, we can see that each replica state is a T-tree. 


A refinement mapping $\refmap$ is given as follows:

Given a replica state $\sigma$ that is a T-tree. Assume that $\sigma = \{ w_1,\ldots,w_n\}$, and for each $i$, $w_i = (v_i,tid_i,gv_i)$. Then, the refinement mapping $\refmap(\sigma) = (l,T)$, where $l$ is obtained by walking $\sigma$ and read ghost value of each T-characters according to $<_t$, and $T = \{ gv_i \vert v_i = nil \}$.

Our proof proceeds as follows:

\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] Let us prove $\mathsf{ReplicaStates}$: Since every operation is appended to the linearization when it executes atSource it clearly follows, the linearization order is consistent with visibility order. Then, by the causal delivery assumption, the order in which effectors are applied at a given replica is also consistent with the visibility order. Let $\alinord_1$ be the projection of linearization order into labels of effectors applied in a replica $\arep$, and $\alinord_2$ be the order of labels of effectors applied in replica $\arep$. By Lemma \ref{lemma:given two sequence consistent with visibility order, one can be obtained from the other}, $\alinord_2$ can be obtained from $\alinord_1$ by several time of swapping adjacent pair of concurrent operations. Since the effector of {\tt addBetween} only do set union, and the effector of {\tt remove} only set the value into $nil$,  
    
    We have already proved that effector of concurrent operations commute. Therefore, we know that $\mathsf{ReplicaStates}$ is an inductive invariant.
