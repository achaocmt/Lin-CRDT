...
\subsection{List with {\tt addAt} Interface}
\label{subsec:list with addAt interface}

~\cite{AttiyaBGMYZ16} use another interface for list and is shown below: 
...
In ``strong list specification'' of ~\cite{AttiyaBGMYZ16}, a transitive, irreflexive and total order over all input values called list-order is used to record the list content where each replica should converge into. For each operation, its return value will be checked w.r.t this list-order.

The return value of {\tt addAt} is a ``local information'' of replica state, and seems play no role when considering \crdtlin{}. \autoref{fig:an example history shows that addAt should not have return value} shows an example history $h$, which is a possible history of RGA with {\tt addAt} interface. The possible linearizations of $h$ are $\alabellong[{\tt addAt}]{a,1}{a}{} \cdot \alabellong[{\tt addAt}]{b,1}{b}{} \cdot \alabellong[{\tt read}]{}{a \cdot b}{}$ and $\alabellong[{\tt addAt}]{b,1}{b}{} \cdot \alabellong[{\tt addAt}]{a,1}{a}{} \cdot \alabellong[{\tt read}]{}{a \cdot b}{}$. Take $\alabellong[{\tt addAt}]{b,1}{b}{} \cdot \alabellong[{\tt addAt}]{a,1}{a}{} \cdot \alabellong[{\tt read}]{}{a \cdot b}{}$ for an example. After we do $\alabellong[{\tt addAt}]{a,1}{a}{}$, the list should contain both $a$ and $b$, and the return value $a$ could be ignored here. 

\begin{figure}[!h]
  \centering
  \includegraphics[width=0.6 \textwidth]{figures/addAtshouldnotHaveReturnValue.pdf}
\vspace{-10pt}
  \caption{An example history shows that {\tt addAt} should not have return value.}
  \label{fig:an example history shows that addAt should not have return value}
\end{figure}

Therefore, let us consider the following interface of list: 
...
\subsection{The Sequential Specification of List with {\tt addAt} Interface}
\label{subsec:the sequential specification of list with addAt interface}

Each abstract state $\abstate = l$ contains a sequence $l$ of elements of a given type. The element $l$ is the list of all input values that are not removed. The sequential specification $\specAddat$ of list with {\tt addAt} interface is given with the transitions as follows:
\[
  \begin{array}{rcl}
    \big(\ (l_1 \cdot l_2\big)\ |\ a\text{ is fresh}\, , \vert l_1 \vert = k-1 \big)
...
Method $\alabelshort[\mathtt{addAt}]{a,k}$ puts $a$ at position $k$ in $l$, assuming that each value is put into list at most once. Method $\alabelshort[\mathtt{remove}]{a}$ adds $a$ into $T$, hence removing $a$ from the list for subsequent calls to the $\mathtt{read}$ method. Thus, $\alabellong[\mathtt{read}]{}{s}{}$ returns the list content excluding any element appearing in $T$. Assume that the initial value of list is $(\circ,\emptyset)$, and $\circ$ are never removed. When the context is clear, in $\ensuremath{\tt read}$ operation, we will omit $\circ$ in return value. 




\subsection{RGA is not \crdtlinearizable{} w.r.t $\specAddat$.}
\label{subsec:RGA is not RA-linearizable w.r.t specAddat}

\autoref{fig:an example history shows that RGA is not RA-linearizable w.r.t specAddat} shows an example that is a history of RGA and is not \crdtlinearizable{} w.r.t $\specAddat$. Here we assume that $\ats_a < \ats_b < \ats_c < \ats_d < \ats_e$. 

\begin{figure}[!h]
  \centering
  \includegraphics[width=0.9 \textwidth]{figures/RGAwithaddAtNotRALin.pdf}
\vspace{-10pt}
  \caption{An example history shows that RGA is not \crdtlinearizable{} w.r.t $\specAddat$.}
