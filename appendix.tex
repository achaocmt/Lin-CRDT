...
\item[-] For case when replica $\arep$ do generator of a {\tt write} operation $\alabel$ and then apply its effector: let $(a,V')$ be the effector of $\alabel$, let $S$ and $S'$ be the replica state of replica $\arep$ of $\aglobalstate$ and $\aglobalstate'$, respectively. Obviously $S' = \{ (a,V') \}$.
    ...
    Since $\alabel$ is greater than any operations whose effectors are in $S$ w.r.t the visibility relation, Annotation2 still holds for $S'$. %in $\aglobalstate'$.
...
\item[-] For case when replica $\arep$ apply effector $(a,V')$ of a {\tt write} operation $\alabel$ originated in a different replica: We only need to prove Annotation2. Let $S$ and $S'$ be the replica state of replica $\arep$ of $\aglobalstate$ and $\aglobalstate'$, respectively.
    ...
    By the causal delivery assumption, for each operation $\alabel'$ such that $(\alabel,\alabel') \in \avisord$, the effector of $\alabel$ has not been applied in $S'$, and then, the effector of $\alabel$ has not been applied in $S$. Therefore, Annotation2 still holds in $S'$. %$\aglobalstate'$.
...
\[
  ...
  {(\gstates, \avisord, \msgs, \vmsgs) \xrightarrow{} (\gstates, \avisord, \msgs \cup \{ (\astate,\alabelset) \}, \vmsgs )}
\]

The third rule APPLY describes a replica $\arep$ with local configuration $(\alabelset, \astate_1)$ applying a message $(\astate_2,Ls)$. This transition results in modifying the state of $\arep$ to $\astate_3 = \alabelshort[{\tt merge}]{\astate_1,\astate_2}$ and adding $Ls$ to the set of operations of local configuration of replica $\arep$. 

\[
  \inferrule[\text{\sc Apply}]
  {\gstates(\arep) = (\alabelset, \astate_1) \\ (\astate_2,Ls) \in \downstreams \\ \alabelshort[{\tt merge}]{\astate_1,\astate_2} = \astate_3} 
...
We use the same $\mathsf{Refinement}$ and $\mathsf{\CRDTLinshort{}}$, and propose a new $\mathsf{ReplicaStates}$ as follows: 

\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] $\mathsf{ReplicaStates}$: We require that, for each replica $\arep$ with local configuration $(\alabelset,\astate)$, the state $\sigma$ is obtained by merging the ``virtual message'' of operations in $\alabelset$ in the order defined by $\alinord$.
\end{itemize} 

Note that, each ``virtual message'' of an operation is a replica state. 

As in Section \ref{sec:proofs}, we identify two general classes of CRDT implementations which differ in the way in which the linearization $\alinord$ is extended when executing operations at the origin replica. One class of objects, including PN-counter, LWW-register, multi-value register and 2P-set, admit execution-order linearizations; while the other class of objects, including LWW-element Set, admit timestamp-order linearizations. 

We need to prove that the domain of replica state is a semilattice, the order of the semilattice is given in {\tt compare} method, and $\alabelshort[\mathtt{merge}]{x,y}$ returns the least upper bound of semilattice. Then, we can imply $\mathsf{ReplicaStates}$. 



\subsection{PN-Counter Implementation and its Proof}
\label{subsec:PN-counter implementation and its proof} 

The state-based PN-counter implementation of \cite{ShapiroPBZ11} is given in Listing~\ref{lst:state-based emulation of state-based object}. 


\begin{minipage}[t]{1.0\linewidth}
\begin{lstlisting}[frame=top,caption={state-based emulation of state-based object},
captionpos=b,label={lst:state-based emulation of state-based object}]
  payload S ( the state-based payload )
  initial initial payload of S
