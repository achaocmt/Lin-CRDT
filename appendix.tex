...
\subsection{Definitions and Proofs in Subsection \ref{subsec:t0-linearizability and t1-linearizability}}
\label{subsec:appendix definitions and proofs in subsection subsec t0-linearizability and t1-linearizability}
...
\begin {proof} 
Given a distributed linearizable history $h$ and assume that $\mathit{lin}$ is a linearization. We need to prove that, each such sequence $\mathit{lin}'$ described below is also a linearization of $h$
...
We prove this by showing that each such $\mathit{lin}'$ can be obtained from $\mathit{lin}$ by several times of swapping a pair of adjacent elements. Our proof proceed in three steps: 

\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] In the first step, we need to prove that, given a linarization $\mathit{lin}$ and a sequence $\mathit{lin}'$ consistent with visibility relation of $h$, if $\mathit{diff}(\mathit{lin},\mathit{lin}') \neq \emptyset$, there exists $(o_1,o_2) \in \mathit{diff}(\mathit{lin},\mathit{lin}')$, such that $o_1$ and $o_2$ are concurrent, and $o_1$ and $o_2$ are adjacent in $\mathit{lin}$. 
...
\item[-] In the second step, we need to prove that, given a linearization $\mathit{lin}$ and $o_1,o_2 \in \mathit{lin}$, such that $o_1$ and $o_2$ are concurrent and adjacent in $\mathit{lin}$, sequence $l$ is obtained from $\mathit{lin}$ by swapping $o_1$ and $o_2$, then, $l$ is also a linearization. 
    
    Let $o_1 = (\ell_1,\mathit{id}_1,S_1)$ and $o_2 = (\ell_2,\mathit{id}_2,S_2)$. Since $o_1$ and $o_2$ are concurrent, we know that $\mathit{id}_1 \notin S_2 \wedge \mathit{id}_2 \notin S_1$. Assume $\mathit{lin} = l_1 \cdot o_1 \cdot o_2 \cdot l_2$. Then, to prove the second step, it is enough to prove that, if in the abstract state of $\mathit{spec}$, we have  
\end{itemize}  
