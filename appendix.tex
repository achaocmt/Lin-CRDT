...
\item There exists a function $\gconfres: \labels^* \rightarrow \abstates$ (Here $\gconfres$ is shorten for global conflict resolution). Given sequence $s \in \labels^*$, $\gconfres(s)$ is defined as follows: assume $\abstate_0$ is the initial abstract state of \Spec{}, 
...
Moreover, we require function $\gconfres$ to satisfy the following property: 

\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] Given two sub-sequences $s_1,s_2$ of $\aseqord\!\downarrow_{\updates}$, if $s_2$ contains more operations than $s_1$, and both $\gconfres(s_1)$ and $\gconfres(s_2)$ are defined, then, $\gconfres(s_1) \xrightarrow{ s_2 - s_1 }^* \gconfres(s_2)$, where $s_2 - s_1$ denotes the sequences obtained from $s_2$ by removing elements of $s_1$.
%\item[-] Given two sequences $s_1,s_2$, if they contain same elements and are both consistent with \avisord{}. Then, either both $\gconfres(s_1)$ and $\gconfres(s_2)$ are undefined, or $\gconfres(s_1) = \gconfres(s_2)$.
\end{itemize}
...
Let us explain Definition \ref{definition:ralinearizability1 with non-deterministic specifications} from two aspects: we use $\gconfres(s)$ to remember our choice for operation sequence $s$, and 
...
\item[-] Unique Choice in Specification: We should get rid of histories that the history has several operations that use ``Inconsistent non-deterministic choice''. 
...
To get rid of such histories, we use the condition $\gconfres(s_1) \xrightarrow{ s_2 - s_1 }^* \gconfres(s_2)$, which represents that, a ``bigger state'' follows the same choice in a ``smaller'' state.
\end{itemize}
...
\begin{itemize}
\item[-] We introduce a function $\igconfres: \labels^* \rightarrow \states$ to explicitly record the consequence of applying downstreams. Given a sequence $s$ of dwonstream, $\igconfres(s)$ is the local state obtained by applying $s$. 

\item[-] From each $\igconfres(s)$, we generate a abstract state $\gconfres(s)$, and the refinement mapping $\refmap$ maps $\igconfres(s)$ into $\gconfres(s)$. Or we can say, $\refmap{}(\igconfres(s)) = \gconfres(s)$.  

\item[-] Similar as in Section \ref{sec:proofs}, we prove that any two downstreams that correspond to two ``concurrent'' operations commute. 
\end{itemize}
