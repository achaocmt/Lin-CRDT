...
captionpos=b,label={lst:wooki algorithm}]
  ...
  integrateIns(@|$c_p,c,c_n$|@)
    ...
    if @|$S' = \epsilon$|@
      ...
      if (@|$c<_{id} F[0]$|@)
        integrateIns(@|$c,c_p,F[0]$|@)
      else
        while (@|$i < \vert F \vert -1 \wedge F[i] <_{id} c$|@) do
            i = i+1
        if (@|$i = \vert F \vert -1 \wedge F[i] <_{id} c$|@)
            integrateIns(@|$c,F[i],c_n$|@)
        else
...
To do $\alabelshort[{\tt addBetween}]{a,b,c}$, we first ensure that $a$ and $c$ are adjacent in $string_s$ and $b$ is not in $string_s$. Then, we generate a W-character $w$ for value $b$, and calls method $integrateIns(w,c_p,c_n)$ to put $w$ between $c_p$ and $c_n$, which are the W-characters of $a$ and $c$ in $string_s$, respectively.
...
For example, in \figurename~\ref{fig:an execution of tree-doc, the local state of replica r1 after execution, and the T-identifiers of T-characters}, the T-identifier of $w_f$ is $0 \cdot (0,id_f)$, which represent that $w_f$ is a ``left son'' of the root's ``left son (could be a major node)''; the T-identifier of $w_g$ is $(0,id_b) \cdot (1,id_g)$, which represent that $w_g$ is a ``right son'' of minor node $w_b$. Note that, as shown in \figurename~\ref{fig:an execution of tree-doc, the local state of replica r1 after execution, and the T-identifiers of T-characters}, the corresponding tree of T-characters is not a binary tree.
...
If $\alabelshort[{\tt addBetween}]{a,b,c}$ calls $\alabelshort[{\tt integrateIns}]{c_p,c_b,c_n}$, and that, for each time, we find $d_{min}^i$ and then recursively calls $\alabelshort[{\tt integrateIns}]{c_i,c_b,c'_i},\ldots$. Then the degree of $c_i$ and $c'_i$ is chosen from $\{ d_p,d_c,d_{min}^1,\ldots d_{min}^i \}$, where $d_p$ and $d_c$ is the degree of $c_p$ and $c_n$, respectively. 
\end{lemma}


The following lemma states that, given two W-characters that are degree-$i$-adjacent in $string_s$, then, they are ordered by $<_id$. 
