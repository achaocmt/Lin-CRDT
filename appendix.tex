...
$\mathit{payload}$ set $S$; \\
$\mathit{initial}$ $\emptyset$; \\

$add(a)$ \\
\ \ $\mathit{atSource}$: \\
\ \ \ \ let $(c,r')$ be maximal time-stamp of $S$; \\ 
...
\item[-] $\Sigma = \{ S \vert$, $S$ is a set, each item of $S$ is of the form $(a,\mathit{ts})$ with $a \in D$ and $\mathit{ts} \in \mathbb{N} \times \mathbb{R} \}$. $\Sigma_0 = \emptyset$. 
...
\item[-] $\mathit{do}(S,\mathit{add},a) = (S \cup \{ (a, \mathit{ts}) \},(a,\mathit{ts}))$, where $\mathit{ts} = (\mathit{max} \{ c \vert (\_,(c,\_)) \in S \} +1,r)$.  

\item[-] If $\exists \mathit{ts}', (a,\mathit{ts}') \in S$, then $\mathit{do}(S,\mathit{rem},a) = (S \setminus S_1, S_1)$, where $S_1 = \{ (a,\mathit{ts}) \vert (a,\mathit{ts}) \in S \}$.
    
\item[-] $\mathit{do}(S,\mathit{read}) = (S,\{ a \vert \exists \mathit{ts}, (a,\mathit{ts}) \in S \})$. 

\item[-] $\mathit{receive}(S,(a,\mathit{ts})) = S \cup \{ (a,\mathit{ts}) \}$, 

\item[-] $\mathit{receive}(S,S_1) = S \setminus S_1$, 
\end{itemize} 
