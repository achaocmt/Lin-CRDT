...
\gpnote{Maybe we have an overlap of IDs}
...
Here we assume that when a item is removed, it will never be added again. The sequential specification $\mathit{set}_s$ of set is given as follows:  Let $\mathit{state}$ be a set and each its element $(a,S,f)$ is a tuple of a data $a$, a set $S$ of identifiers and a flag $f \in \{ \mathit{true},\mathit{false} \}$.

\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] $\{ \mathit{state} = S \wedge (a,\_,\_) \notin S \}$ $(\mathit{add}(a),\mathit{id})$ $\{ \mathit{state} = S \cup \{ (a,\{ \mathit{id} \},\mathit{true}) \} \}$.
\item[-] $\{ \mathit{state} = S \wedge (a,S_1,\mathit{true}) \in S \}$ $(\mathit{add}(a),\mathit{id})$ $\{ \mathit{state} = S[a: (S_1 \cup \{ \mathit{id} \},\mathit{true}) ] \}$.
\item[-] $\{ \mathit{state} = S \wedge (a,S_1,\mathit{true}) \in S \wedge S_1 \subseteq S_2 \}$ $(\mathit{rem}(a),S_2)$ $\{ \mathit{state} = S[a: (S_1,\mathit{false}) \}$.
...
A label is an update label if in its specification the state before
...
\gpnote*{We consider single-object only in this section}
...
\gpwarning*{}{We have to say that we distinguish query and update labels, perhaps define formally (for queries the visibility is important, updates change the state, but the visibility might not be important.)}

\begin{definition}[\CRDTLin{}]
\label{definition:distributed linearizability}

A history $h = (\alabelset,\arepord,\avisord)$ is \CRDTLin{} w.r.t. a \gp{First mention?}{deterministic} sequential specification \Spec{}, if there exists a specification history sequence $(\alabelset, \aseqord, \avisord) \in \Spec{}$, called the \crdtlin{} of $h$, where we can note that the set of labels and the visibility relation are identical, and such that
\gpwarning*{}{This definition has to be worked out again}
\begin{enumerate}[(i)]
% \item The elements of $\mathit{lin}$ is generated from the operations of $h$: each operation $o = (m(a) \Rightarrow b,i,\_)$ is transformed into $(m(a) \Rightarrow b,i,S)$ with $S$ set of identifiers of operations visible to $o$.
...
\item the projection of $\aseqord$ to only \emph{update} labels is also in $\Spec$, and
\item for each query label $\alabel$, $\aseqord\!\uparrow_{\avisord^{-1}(\alabel)}\! \cdot\ \alabel \in \Spec$.
\end{enumerate}

% , where the notation $h \uparrow_{x}$ is obtained from $h$ by projecting the operations and relations into operations of object $x$

\gpfatal{Fix} In a nutshell this definition requires that for a given
