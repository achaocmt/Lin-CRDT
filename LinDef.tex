...
$\alabel_{\mathsf{up}} \in \Updates$ and into $\alabel_{\mathsf{qr}} \in \Queries$.
...
\begin{definition}[\HWLin{}]
  A history $h = (\alabelset,\arepord,\avisord)$ is related by
  \HWLin{} to a sequential specification \Spec{} if there exists a
  history $(\alabelset, \aseqord, \bigcup_{\alabel \in \alabelset}
  \{\alabel\} \times \aseqord^{-1}(\alabel)) \in \Spec{}$, called the
  \hwlinearization{} of $h$, where we require $\aseqord$ to be
  consistent with $\avisord$ (i.e. $(\aseqord \cup \avisord)^+$ is acyclic).
  Notice that the sets of labels are the same, and moreover, we
  require the visibility relation of the specification history to be
  given by the sequential order $\aseqord$, meaning that for each
  operation its visibility is the prefix in $\aseqord$ up to that
  operation.
\end{definition}
...
Thus, we shall relax the definition to accept histories where
not all update operations are required to be seen by every query
operation.

\gpnote*{}{Not sure about the details here. Gotta check again.}
\begin{definition}[\CRDTLin{}]
  ...
  a \gpnote*{First mention?}{deterministic} sequential specification
...
\item the projection of $\aseqord$ to only \emph{update} labels is
  related by \hwlin{} with $\Spec$, i.e. $\aseqord/_{\Updates}$ is
  \hwlinear{}, and
\item for each query label $\alabel_{\mathsf{qr}}$,
  $\aseqord\!\uparrow_{\avisord^{-1}(\alabel_{\mathsf{qr}})}\!
  \cdot\ \alabel_{\mathsf{qr}}$ is related by \hwlinear{} w.r.t.
  $\Spec$.
  Where the notation $\aseqord\!\uparrow_{S}$ represents the
  restriction of the order $\aseqord$ to the elements in the set $S$.
\end{enumerate}
\end{definition}
...
A set $H$ of histories is \crdtlinearizable{} (or \hwlinearizable{}) w.r.t
a \gpnote*{}{deterministic} sequential specification \Spec, if each of
its histories is \crdtlinearizable{} (resp. \hwlinearizable{}) w.r.t. \Spec.
