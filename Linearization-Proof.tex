...
\item[-] $h$ is a history. We also require that the visibility relation be transitive. 
...
\item[-] $\mathit{map} \subseteq \mathbb{MID} \times 2^{\mathbb{OID}}$ maps each message $\mathit{mid}$ to a set $S_1$ of operations. $S_1$ represents the set of operations whose information are contained in $\mathit{mid}$. 
...
\item[-] $C_2: \forall r$, $C(r) = \mathit{apply}(\mathit{lin},\mathit{vd}(r))$. 
\end{itemize} 

The function $\mathit{apply}(\mathit{lin},S)$ returns a local state by applying ``virtual messages'' of operations in $S$ according to total order $\mathit{lin}$. Let $\mathit{vd}(r) = \{ i \vert (i,j) \in h.\mathit{vis}, j$ is of replica $r \} \cup \{ i \vert (i,r) \in \mathit{del} \}$. Here for each update operation $o$ of $h$, we need to define a local state $\mathit{ds}(o)$, which is the ``virtual messages'' of $o$. Note that state-based CRDT send message randomly, instead of each message for a update operation.
