...
\begin{lstlisting}[caption={Pseudo-code of the or-set CRDT}, captionpos=b,label={lst:or-set}]
  ...
  remove(a) :
    atSource :
      precondition : @|$\exists$|@ ts, (a,ts) @|$\in$|@ S 
      let S@|$_1$|@ = @|$\{$|@ (a,ts) @|$\in$|@ S @|$\}$|@
    downStream(S@|$_1$|@) :
      precondition : 
      S = S @|$\setminus$|@ S@|$_1$|@ 
  ...
  read() :
    return {a @|$\vert$|@ @|$\exists$|@ ts, (a,ts) @|$\in$|@ S} 
\end{lstlisting}


In the downstream of a $\alabellongNoret[\mathit{add}]{\argv}$ operation, an tuple $(\argv,\mathit{ts})$ will be added to the local state; while in the downstream of a $\alabellongNoret[\mathit{add}]{\argv}$ operation, a set $S_1$ will be removed from the local state. We call such tuple $(\argv,\mathit{ts})$ or set $S_1$ the downstream content of the corresponding operation. The following is the condition $C_2$ for or-set implementation.
...
\begin{itemize}
\setlength{\itemsep}{0.5pt}
\item[-] If $\alabel = \alabellongNoret[\mathit{add}]{\argv}$: Assume $\alabel$ is generated for replica $\arep$. Let $c = \mathsf{max}\{ c' \vert \exists \alabel'$, such that $(\alabel', \alabel) \in \avisord$, and $(\_,(c',\_))$ is the downstream content of $\alabel' \}$. Then, the downstream content of $\alabel$ is $(a,(c+1,r))$. 

\item[-] If $\alabel = \alabellongNoret[\mathit{remove}]{\argv}$: Let $S' = \{ \alabel' = \alabellongNoret[\mathit{add}]{\argv} \vert (\alabel',\alabel) \in \avisord, \neg \exists \alabel'' = \alabellongNoret[\mathit{remove}]{\argv}, (\alabel',\alabel''), (\alabel'',\alabel) \in \avisord \}$. Then, the downstream content of $\alabel$ is the set of downstream contents of operations of $S'$.  
\end{itemize} 
