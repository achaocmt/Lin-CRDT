...
In this section we formalize the intuitions developed in \sectionautorefname~\ref{sec:overview}. We define the semantics of CRDT objects (\S~\ref{ssec:semantics}), specifications (\S~\ref{subsec:sequential specification}), and our notion of RA-linearizability (\S~\ref{subsec:definition of distributed linearizability}).
...
in \figureautorefname~\ref{fig:sem-dom}.
%
...
\figureautorefname~\ref{fig:crdt-opsem}.
...
with object compositions (see \sectionautorefname~\ref{sec:compositionality}).
...
of an execution (trace) can be found in \figureautorefname~\ref{fig:or-set-not-lin} while an example of a history
can be found in \figureautorefname~\ref{fig:rga-history}.
...
will use Hoare-style preconditions and write\\[3pt]
\centerline{
\(\big(\abstate\ |\ \apre(\abstate)\big) \specarrow{\alabel}
  \abstate'\)
}\\[3pt]
In this way, a sequential specification is the set
...
\begin{example}[Sequential Specification of a Counter]
  ...
  be an integer, and the transitions are given as follows:\\[3pt]
\centerline{
\(
\small
...
\begin{example}[Sequential Specification of RGA 
  ...
  \[\small
    \begin{array}{rcl}
      \big(\ (l_1 \cdot b \cdot l_2,T\big)\ |\ a\text{ is fresh}\ \big)
      & \specarrow{\alabelshort[\mathtt{addAfter}]{b,a}}
      & (l_1 \cdot b \cdot a \cdot l_2,T)\\
      \big(\ (l,T)\ |\ b\ \text{occurs in}\ l\ \big)
      & \specarrow{\alabelshort[\mathtt{remove}]{b}}
      & (l,T \cup \{b\})\\
      (l,T)
 ...
 \end{array}
  \]
The method $\alabelshort[\mathtt{addAfter}]{b,a}$ puts $a$ immediately
  after $b$ in $l$, assuming that each value is put into list at
  most once.
  Method $\alabelshort[\mathtt{remove}]{b}$ adds $b$ into $T$,
  hence removing $b$ from the list for subsequent calls to the
  $\mathtt{read}$ method.
...
history. This rewriting introduces update operations $\alabelshort[{\tt add}]{\mathit{a,id}}$, for some identifier $\mathit{id}$, instead of simply $\alabelshort[{\tt add}]{\mathit{a}}$,
and $\alabelshort[\mathtt{remove}]{a,S}$, for some set of identifiers $S$, instead of $\alabelshort[{\tt remove}]{\mathit{a}}$,
and a new query operation $\alabelshort[{\tt readIds}]{\mathit{a}}$ that returns a set of identifiers. These operations are specified as follows.
...
\[\small
  \begin{array}{rcl}
    ...
    \big(\ \abstate\ |\ \mathtt{id}\ \text{does not occur in } \abstate\ \big)
             & \specarrow{ \alabelshort[{\tt add}]{\mathit{a,id}} }
    & \abstate \cup \{ (a,\mathit{id}) \}\\
    \abstate
    & \specarrow{\alabellong[\mathtt{read}]{a}{ A }{}}
    & \abstate\
      \begin{array}{c}
        [\text{with}\ A = \{ a\ \vert\ \exists\ \mathit{id}, (a,\mathit{id}) \in \abstate \}]
      \end{array}
  \end{array}
\]
Here $\alabellong[{\tt readIds}]{\mathit{a}}{\mathit{S}}{}$ returns the set of pairs
with data $a$, $\alabelshort[{\tt remove}]{\mathit{a,S}}$ removes $S$ from the
abstract state, $\alabelshort[{\tt add}]{\mathit{a,id}}$ puts $\{ (a,id) \}$
into the abstract state, and $\alabellong[{\tt read}]{}{\mathit{A}}{}$ returns
...
We discuss this issue further in \sectionautorefname~\ref{subsec:time order of execution as linearization}.
%
...
\label{ex:qur-orset}
As shown in \figureautorefname~\ref{fig:or-set-lk-rem}, the query-update rewriting for OR-Set 
is defined by:  $\gamma( \alabelshort[\mathtt{add}]{a} ) = \alabelshort[\mathtt{add}]{a,\mathit{id}}$ where $\mathit{id}$ is a unique identifier,
