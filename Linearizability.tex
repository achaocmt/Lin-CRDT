...
%
We will generally omit writing this value in labels and operations
since this value is of no importance.
%
...
each operation a downstream $\effector\in [\states \rightarrow
\states]$, which is a transformer of replica states.
...
\avisord$.
This transition, labeled by the action $\dwn{\arep}{\alabel}$ (we may ignore the index $\arep$ when it is not important), results in modifying the state of $\arep$ to
...
\begin{example}[Sequential Specification of List ($\mathtt{addAfter}$)]
  \label{definition:sequential specification of or-set} 
  Each abstract state $\abstate = (l,T)$ contains a sequence $l$ and a
  set $T$.
