...
For uniformity, we assume that $\ts$ is a distinguished value $\bot$ for methods $\amethod$ that don't generate timestamps, e.g., the method {\tt remove} of the RGA object. We may omit the timestamp when it is not important.
...
each query-update is decomposed into a label representing the query part and another label representing the update part. This rewriting may introduce new labels, which do not correspond to original operations of the considered object, but which have to be taken into account by the specification. TODO EXPLAIN THE OR-SET CASE.
