...
As illustrated in \figureautorefname~\ref{fig:or-set-lk-rem}, for
...
%
Then abstract state $\abstate$ is a set of tuples $(a,id)$, where $a$
...
%
For instance the Wooki~\cite{DBLP:conf/icdcs/PreguicaMSL09} CRDT of a
List-like object provides a method $\mathtt{addBetween(a, b, c)}$
which inserts $\mathtt{b}$ between $\mathtt{a}$ and $\mathtt{c}$
to be present in the list.
%
In contrast with RGA, where the method $addAfter(a, b)$
explicitly adds the element $\mathtt{b}$ after $\mathtt{a}$, in Wooki
many positions are possible if $\mathtt{a}$ and $\mathtt{c}$ are not
adjacent.
%
...
representing the query part a another label representing the update
part.
%
