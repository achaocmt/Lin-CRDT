...
In that case the meta-variable $\ts$ adopts the value $\bot$. We extend the order
...
transition relation. For readability, we use $\aglobalstate \xrightarrow{\anact} \aglobalstate'$
to denote a transition $(\aglobalstate,\anact,\aglobalstate')\in\rightarrow$.
...
An execution of the object $\aobj$ is a sequence of transitions $\aglobalstate_0\xrightarrow{\anact_0}\aglobalstate_1\xrightarrow{\anact_1}\ldots$.
A \emph{trace} $\atrace$ is the sequence of actions $\anact_0\anact_1\ldots$ labeling the transitions of an execution.
...
\begin{example}[sequential specification of List with add-after interface]
\label{definition:sequential specification of or-set}
...
\fxwarning[nomargin, inline]{REFER TO AN EXAMPLE FROM THE OVERVIEW}
...
well.}
\fxwarning[nomargin, inline]{EXPLAIN THE OR-SET CASE.}
