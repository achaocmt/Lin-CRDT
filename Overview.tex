...
at most one element, contrary to the final set we obtain:
$\mathtt{\{a, b\}}$.
...
%
We will then require that if we look only at update operations, they
can be totally ordered (linearized) such that the resulting states of
the object can be explained by executing the \emph{specification} of
these updates on the object.
%
Essentially, reachable states of the implementation can be reached by
a linearization of the updates when considering the specification. 
%
However, this cannot be replicated for query operations, since the
system model allows for updates to arrive to different replicas in
different order. 
%
