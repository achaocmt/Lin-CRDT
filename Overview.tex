...
\begin{figure}[t]
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize,caption={Replicated Growing Array (RGA) CRDT pseudo-code (adapted from~\cite{ShapiroPBZ11}).},captionpos=b,label={lst:rga}]
...
The execution model for an operation \lstinline|op(v)| on a replicated object is depicted in Figure~\ref{fig:sys-mod} and it proceeds as follows:
\begin{itemize}
...
in Listing~\ref{lst:rga}, where we ignore the lines starting with the
annotation \lstinline|//@| for the moment (this is a version of
the RGA algorithm introduced in~\cite{AttiyaBGMYZ16}).
%
...
\item The keyword \lstinline|payload| introduces the state that is
  ...
  declaration in an object oriented language such as Java. In the
...
\begin{figure}[t]
  \centering
  \includegraphics[width=.7\textwidth]{figures/RGA-Trace}
% \vspace{-10pt}
